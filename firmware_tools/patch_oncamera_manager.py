...
HA_PAGE = b"""<html><head><meta name=viewport content="width=device-width"><link rel=icon href=data:,><style>body{font:15px Arial;max-width:760px;margin:30px auto;line-height:1.5;color:#234}a.button{display:inline-block;padding:10px 16px;margin:5px;background:#345;color:white;text-decoration:none}code{background:#eee;padding:2px 5px}</style></head><body><p><a href=main.asp>&larr; Back</a></p><h1>Home Assistant / MQTT</h1><p><b>Optional integration.</b> The camera works without MQTT. If installed, the Home Assistant add-on receives stock HTTP motion events and publishes MQTT discovery, motion, availability and JPEG snapshots. No cloud is used.</p><ol><li>Install and start the optional Gigaset Camera MQTT add-on in Home Assistant.</li><li>Configure an HTTP server slot with the Home Assistant IP, port <code>8766</code>, and Authorization <b>No</b>.</li><li>Enable HTTP event alarm delivery, select that slot, choose Always, and use URL <code>motion?token=YOUR_TOKEN</code>.</li></ol><p><a class=button href=motiondect2.asp?p=e>1. HTTP server</a><a class=button href=motiondect2.asp?p=h>2. Motion delivery</a><a class=button href=motiondect2.asp?p=m>3. Motion zones</a></p></body></html>"""

LIVE_PAGE = b"""<html><head><meta name=viewport content="width=device-width"><link rel=icon href=data:,><style>body{margin:0;background:#26343d;color:white;font:14px Arial}nav{padding:12px;background:#345}a{color:white;margin-right:18px}main{text-align:center;padding:12px}img{width:min(100%,854px);height:auto;background:#111}code{user-select:all}</style></head><body><nav><a href=main.asp>&larr; Back</a><a href=motiondect2.asp?p=s>Stream settings</a></nav><main><img src=../stream.jpg><p>Primary RTSP: <code id=p></code><br>Secondary RTSP: <code id=s></code></p></main><script>var b='rtsp://'+location.hostname+':80/';p.textContent=b+'live_h264.sdp';s.textContent=b+'live_h264_1.sdp'</script></body></html>"""
